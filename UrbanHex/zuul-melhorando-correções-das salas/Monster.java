...
public class Monster {
    ...
    public synchronized void spawnAdjacentImmediate(Room playerRoom) {
        ...
        //cancela esses timers pendentes e coloca o monstro na hora
        cancelPendingSpawn();
     ...
     * Se já houver monstro no jogo nada acontece.
     */
    public synchronized void spawnAdjacentWithDelay(Room alertRoom, long delayMillis) {
        ...
        // se já houver monstro (ou outro timer ativo), não agenda
        if (game.getMonsterRoom() != null) return;

        // Cancela qualquer timer anterior antes de agendar um novo (em teoria)
        cancelPendingSpawn();
        ...
        pendingTimer = new Thread(() -> {
            ...
            synchronized (Monster.this) {
                ...
                if (game.getCurrentRoom() == alertRoom && game.getMonsterRoom() == null) {
                    // coloca o monstro na sala do jogador (entrada triunfal do monstro cheio de tentaculos <-- apagar isso)
                    ...
                    // jogador saiu da sala ou monstro já foi colocado 
                }
                pendingTimer = null;
            }
        }, "monster-timer");
