...
public class Game
{
    ...
    // sala inicial (primeira sala do jogo) — usada para restringir go outside
     ...
     * nenhuma das salas já tiver saída trancada (evita múltiplas trancas por sala).
    ...
    private void createRooms()
    {
        ...
        Room entrada = new Imagens("na entrada da mansão");
        Room hall = new Imagens("no hall principal");
        Room cozinha = new Imagens("na cozinha");
        ...
        Room biblioteca = new Imagens("na biblioteca");
        ...
        Room quartoMestre = new Imagens("no quarto mestre");
        Room banheiro = new Room("no banheiro");
        Room varanda = new Imagens("na varanda");

        // Sala externa (lado de fora da mansão) — saída para o mundo exterior
        Room outside = new Imagens("do lado de fora da mansão");

        // Conexões térreo (bidirecionais) — reorganizadas conforme solicitado:
        // - a cozinha deve estar na saída west do hall principal
        // - a entrada deve estar na saída south do hall principal
        // - o hall principal deve estar na saída east da cozinha
        // - o corredor que estava em 'west' do hall foi movido para outra saída (aqui: 'east')

        // entrada <-> hall  (entrada north -> hall, hall south -> entrada)
        setBidirectionalExitWithPossibleLock(entrada, "north", hall, "south");

        // hall west -> cozinha  (cozinha east -> hall)
        setBidirectionalExitWithPossibleLock(hall, "west", cozinha, "east");

        // hall north -> corredor1  (corredor1 south -> hall)
        setBidirectionalExitWithPossibleLock(hall, "north", corredor1, "south");

        // hall east -> corredor2  (mudei corredor2 para 'east' do hall)
        setBidirectionalExitWithPossibleLock(hall, "east", corredor2, "west");

        // corredor2 south -> estufa  (estufa north -> corredor2)
        setBidirectionalExitWithPossibleLock(corredor2, "south", estufa, "north");

        // cozinha north -> estufa  (estufa south -> cozinha)
        setBidirectionalExitWithPossibleLock(cozinha, "north", estufa, "south");

        // quartos ligados ao corredor1 (mantidos)
        ...
        // salaoDoCofre e cofre — coloquei salaoDoCofre em "northeast" do hall para evitar sobrescrever 'north'
        setBidirectionalExitWithPossibleLock(hall, "northeast", salaoDoCofre, "southwest");
        ...
        // Conexão para fora: permite usar "go outside" a partir da entrada
        setBidirectionalExitWithPossibleLock(entrada, "outside", outside, "inside");

        // posição inicial
        currentRoom = entrada;
        // marca a primeira sala do jogo explicitamente
        initialRoom = entrada;
        // startRoom agora é o lado de fora: voltar para 'outside' com a relíquia vence o jogo
        startRoom = outside;
    ...
    private void printWelcome()
    {
        ...
        // se sala suporta imagens, mostra
        if (currentRoom instanceof Imagens) {
            ((Imagens) currentRoom).showImage(3000L);
        }
    }
    ...
    private void goRoom(Command command)
    {
        ...
        // Restrição: 'outside' só acessível da primeira sala do jogo (initialRoom)
        if ("outside".equals(direction) && currentRoom != initialRoom) {
            System.out.println("Você não pode sair daqui.");
            return;
        }
